...
print(np.log10(dens_out).min(), np.log10(dens_out).max())
...
def plot_2D_data(output_number, path, X, Y, NY, NX, x_limits, y_limits):
    ...
    plt.figure(figsize=(8, 8))
    mesh = plt.pcolormesh(X, Y, np.log10(dens_out), cmap='jet', shading='auto', vmin=vmin, vmax=vmax)
    plt.scatter(xp, yp, c='white', s=100, edgecolor='black', label='Planet Position')
...
ani.save(png_dir + 'gas_density_animation_from_png.mp4', fps=15, extra_args=['-vcodec', 'libx264'])

plt.show()

#interval: Especifica el tiempo entre fotogramas en milisegundos. Un interval más alto hace que la animación vaya más despacio, 
# mientras que un interval más bajo hace que la animación vaya más rápido.
#Ejemplo: Un interval de 200 ms significa que hay 200 milisegundos entre cada fotograma.

#fps (frames per second): Especifica el número de fotogramas por segundo en el archivo de video resultante. 
# Un fps más alto hace que la animación parezca más suave, mientras que un fps más bajo puede hacer que la animación se vea entrecortada.
