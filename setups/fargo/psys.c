...
PlanetarySystem *InitPlanetarySystem (char *filename) {
  ...
  real mass, dist, accret, e, a, mu, v_p;  //se agrego e, a, mu, v_p
  ...
  while (fgets(s, 510, input) != NULL) {
	...
	sys->mass[0] = PLANETMASS;
      ...
      if (e < 1.0) {
        ...
        // Órbita hiperbólica
        a = dist / (e - 1);
        v_p = sqrt(mu * (2.0 / dist - 1.0 / a));
      }


      if (ECCENTRICITY < 1.0) {
          sys->x[i] = (real)dist * (1.0 + ECCENTRICITY); // Para órbitas elípticas
          } else {
          sys->x[i] = (real)dist; // Para órbitas hiperbólicas
          }
      ...
      // Cálculo de las velocidades basadas en la excentricidad
      if (ECCENTRICITY < 1.0) {
        // Órbita elíptica
        sys->vy[i] = (real)sqrt(G*(MSTAR+sys->mass[i])/dist) * \
          sqrt((1.0-ECCENTRICITY)/(1.0+ECCENTRICITY)) * cos(INCLINATION);
        sys->vx[i] = -0.0000000000 * sys->vy[i];
        sys->vz[i] = sys->vy[i] * sin(INCLINATION) / cos(INCLINATION);
      } else {
        // Órbita hiperbólica
        real a = dist / (ECCENTRICITY - 1); // Semieje mayor negativo
        sys->vy[i] = (real)sqrt(G*(MSTAR+sys->mass[i]) * (2.0 / dist - 1.0 / a)) * cos(INCLINATION);
        sys->vx[i] = -0.0000000000 * sys->vy[i];
        sys->vz[i] = sys->vy[i] * sin(INCLINATION) / cos(INCLINATION);
      }
